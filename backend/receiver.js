...
const receiveImage = function (details) {
  const { signature, receiverPublicKey, receiverDeviceId } = details;
...
const registerReceiver = function(details) {
  const { signature, receiverPublicKey, receiverDeviceId } = details;
  ...
  return sendMessage(shareQuery).then(function(shareQuery) {
      ...
      return receiveImage(details).then(data => data).catch(err=> {throw err;})
  });
};
module.exports = {
  registerReceiver
